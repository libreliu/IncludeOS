...
int screen_init()
{
    ...
    if(mbox_call(MBOX_CH_PROP) && mbox[20]==DEPTH && mbox[28]!=0)
    {
        ...
        height = mbox[6];
        pitch = mbox[33];
        framebuffer = (char *)(void *)((unsigned long)mbox[28]);
    }
    else
    {
...
{
    psf_t *font = (psf_t *)&_binary__home_includeos_includeos_src_hw_font_psf_start;
    while (*s)
    {   
        // Get the start address of the character.
        unsigned char *glyph = (unsigned char *)&_binary__home_includeos_includeos_src_hw_font_psf_start + font->headersize;
