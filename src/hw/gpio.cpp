...
__uint32_t hw::GPIO::read_peri(volatile __uint32_t *paddr)
{
#ifdef GPIO_DEBUG
    printf("read paddr:%p\n", (void *)paddr);
#endif
...
void hw::GPIO::write_peri(volatile __uint32_t *paddr, __uint32_t value)
{
#ifdef GPIO_DEBUG
...
void hw::GPIO::gpio_set_bits(volatile __uint32_t *paddr, __uint32_t mask, __uint32_t value)
{
    __uint32_t ori = read_peri(paddr);
    __uint32_t val = (ori & ~mask) | (value & mask);
    hw::GPIO::write_peri(paddr, val);
}
...
void hw::GPIO::gpio_func_select(const __uint8_t pin, __uint8_t mode)
{
    volatile __uint32_t *paddr = (volatile unsigned int*)(GPFSEL0 + (pin / 10) * 4);
    __uint8_t shift = (pin % 10) * 3;
    __uint32_t mask = GPIO_FSEL_MASK << shift;
    __uint32_t value = mode << shift;
    hw::GPIO::gpio_set_bits(paddr, mask, value);
}

void hw::GPIO::gpio_set(const __uint8_t pin)
{
    volatile __uint32_t *paddr = (volatile unsigned int*)(GPSET0 + pin / 32);
    __uint8_t shift = (pin % 32);
    hw::GPIO::write_peri(paddr, 1 << shift);
}

void hw::GPIO::gpio_clr(const __uint8_t pin)
{
    volatile __uint32_t *paddr = (volatile unsigned int*)(GPCLR0 + pin / 32);
    __uint8_t shift = pin % 32;
    hw::GPIO::write_peri(paddr, 1 << shift);
}

int hw::GPIO::gpio_read_level(const __uint8_t pin)
{
    volatile __uint32_t *paddr = (volatile unsigned int*)(GPLEV0 + pin / 32);
    __uint8_t shift = pin % 32;
